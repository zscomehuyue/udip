...
public class WarningTerminProcess implements TerminProcess {
    ...
    public boolean process(TerminEventData data) {
        logger.warn("nid:{}[{}:{}]", new Object[]{ArbitrateConfigUtils.getCurrentNid(), data.getPipelineId(), data.getCode() + ":" + data.getDesc()});
        alarmClientService.sendAlarm(ArbitrateConfigUtils.getCurrentNid(), data.getPipelineId(), data.getCode(), data.getDesc());
