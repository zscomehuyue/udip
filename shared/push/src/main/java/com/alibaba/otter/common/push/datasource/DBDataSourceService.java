...
public class DBDataSourceService implements DataSourceService, DisposableBean {
    ...
    private Map<DataSource, JdbcTemplate> jdbcTemplate;
    ...
    public DBDataSourceService() {
        ...
        // 构建第一层map
        dataSources = mapMaker.makeComputingMap(new Function<Long, Map<DbMediaSource, DataSource>>() {
        ...
        jdbcTemplate = new MapMaker().makeComputingMap(new Function<DataSource, JdbcTemplate>() {
            public JdbcTemplate apply(DataSource dataSource) {
    ...
    public JdbcTemplate getJdbcTemplate(long pipelineId, DataMediaSource dataMediaSource) {
        Assert.notNull(dataMediaSource);
        DataSource dataSource = dataSources.get(pipelineId).get(dataMediaSource);
        return getJdbcTemplate(dataSource);
    }

    public JdbcTemplate getJdbcTemplate(DataSource dataSource) {
        Assert.notNull(dataSource);
        return jdbcTemplate.get(dataSource);
    }
