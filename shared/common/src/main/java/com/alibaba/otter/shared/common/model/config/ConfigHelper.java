...
public class ConfigHelper {
     ...
     * 根据DataMedia id得到对应的DataMedia
     */
    public static DataMedia<? extends DataMediaSource> findSourceDataMedia(Pipeline pipeline, Long id) {
        Assert.notNull(pipeline);
        for (DataMediaPair pair : pipeline.getPairs()) {
            if (pair.getSource().getId().equals(id)) {
                return pair.getSource();
    ...
    public static boolean needLoadMedia(Pipeline pipeline, Long tableId, DataMediaType type) {
        List<LoadRoute> routes = pipeline.getRoutes();
        if (CollectionUtils.isEmpty(routes) || null == tableId) {
            return false;
        }
        Optional route = routes.stream().filter((loadRoute) -> {
            if (loadRoute.getTable().getId().equals(tableId)) {
                if ((DataMediaType.INDEX == type && loadRoute.getLoadDataMedia().getSource().getType().isIndex())
                        || (DataMediaType.MQ == type && loadRoute.getLoadDataMedia().getSource().getType().isMq())
                        || (type == loadRoute.getLoadDataMedia().getSource().getType())) {
                    return true;
                }
            }
            return false;
    ...
    // db排出掉通过dryRun=true ，这时不需要插入route表,这时排除某个loader
    public static boolean needLoadMedia(Pipeline pipeline, DataMediaType type) {
        List<LoadRoute> routes = pipeline.getRoutes();
        if (CollectionUtils.isEmpty(routes)) {
            return false;
        }
        Optional route = routes.stream().filter(loadRoute ->
                (DataMediaType.INDEX == type && loadRoute.getLoadDataMedia().getSource().getType().isIndex())
                        || (DataMediaType.MQ == type && loadRoute.getLoadDataMedia().getSource().getType().isMq())
                        || (type == loadRoute.getLoadDataMedia().getSource().getType())
     ...
     * 根据DataMedia id得到对应的DataMedia
     */
    public static DataMedia<? extends DataMediaSource> findDataMedia(Pipeline pipeline, Long tableId, DataMediaType type) {
        Assert.notNull(pipeline);
        for (LoadRoute route : pipeline.getRoutes()) {
            if (route.getTable().getId().equals(tableId)) {
                if ((DataMediaType.INDEX == type && route.getLoadDataMedia().getSource().getType().isIndex())
                        || (DataMediaType.MQ == type && route.getLoadDataMedia().getSource().getType().isMq())
                        || (type == route.getLoadDataMedia().getSource().getType())) {
                    return route.getLoadDataMedia();
                }
            }
        }
        return null;
    ...
    public static LoadRoute findLoadRoute(Pipeline pipeline, Long tableId, DataMediaType type) {
        Assert.notNull(pipeline);
        for (LoadRoute route : pipeline.getRoutes()) {
            if (route.getTable().getId().equals(tableId)) {
                if ((DataMediaType.INDEX == type && route.getLoadDataMedia().getSource().getType().isIndex())
