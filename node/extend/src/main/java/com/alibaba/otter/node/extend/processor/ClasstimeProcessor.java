 ...
 * cuc_classtime_id
 */
public class ClasstimeProcessor implements EventProcessor {

    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        Map<EventColumn, String> map = new ConcurrentHashMap(FieldMapping.CLASS_TIME_INCLUDE_FIELD.size());
        eventData.getColumns().parallelStream().forEach(eventColumn -> {
            if (null != FieldMapping.CLASS_TIME_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.CLASS_TIME_INCLUDE_FIELD.get(eventColumn.getColumnName()));
                map.put(eventColumn, FieldMapping.NULL_VALUE);
            }
        });
        //该表只是使用到宽表中，因此不需要cityId FIXME
        map.put(FieldHelper.createCityColumn(dataMediaPair),FieldMapping.NULL_VALUE);
        eventData.getColumns().clear();
        eventData.setColumns(Arrays.asList(map.keySet().toArray(new EventColumn[FieldMapping.CLASS_TIME_INCLUDE_FIELD.size()])));
