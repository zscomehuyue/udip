...
public class TeacherProcessor implements EventProcessor {

    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        List<EventColumn> list = new ArrayList(FieldMapping.TEACHER_INCLUDE_FIELD.size());
        eventData.getColumns().forEach(eventColumn -> {
            if (null != FieldMapping.TEACHER_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.TEACHER_INCLUDE_FIELD.get(eventColumn.getColumnName()));
