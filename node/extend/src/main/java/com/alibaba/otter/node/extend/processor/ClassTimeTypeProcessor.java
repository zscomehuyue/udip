...
public class ClassTimeTypeProcessor implements EventProcessor {
    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        List<EventColumn> list = new ArrayList(FieldMapping.CLASS_TIME_TYPE_INCLUDE_FIELD.size());
        eventData.getColumns().forEach(eventColumn -> {
            if (null != FieldMapping.CLASS_TIME_TYPE_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.CLASS_TIME_TYPE_INCLUDE_FIELD.get(eventColumn.getColumnName()));
