...
public class SystemParamProcess implements EventProcessor {

    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        Map<EventColumn, String> map = new ConcurrentHashMap(FieldMapping.SYSTEM_PARAM_INCLUDE_FIELD.size());
        eventData.getColumns().parallelStream().forEach(eventColumn -> {
            if (null != FieldMapping.SYSTEM_PARAM_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.SYSTEM_PARAM_INCLUDE_FIELD.get(eventColumn.getColumnName()));
        ...
        eventData.setColumns(Arrays.asList(map.keySet().toArray(new EventColumn[FieldMapping.SYSTEM_PARAM_INCLUDE_FIELD.size()])));
        ...
        eventData.getKeys().forEach(eventColumn -> {
            if (null != FieldMapping.SYSTEM_PARAM_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                ...
                //ID存在冲突的情况
                eventColumn.setColumnValue(eventColumn.getColumnValue() + "_" + FieldHelper.getCityCode(dataMediaPair));
