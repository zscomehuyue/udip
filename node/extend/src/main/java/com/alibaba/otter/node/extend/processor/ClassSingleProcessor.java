...
public class ClassSingleProcessor implements EventProcessor {


    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        Map<EventColumn, String> map = new ConcurrentHashMap(FieldMapping.CLASS_SINGLE_INCLUDE_FIELD.size());
        eventData.getColumns().parallelStream().forEach(eventColumn -> {
            if (null != FieldMapping.CLASS_SINGLE_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.CLASS_SINGLE_INCLUDE_FIELD.get(eventColumn.getColumnName()));
        ...
        eventData.setColumns(Arrays.asList(map.keySet().toArray(new EventColumn[FieldMapping.CLASS_SINGLE_INCLUDE_FIELD.size()])));
