...
public class GradeProcessor implements EventProcessor {


    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        Map<EventColumn, String> map = new ConcurrentHashMap(FieldMapping.GRADE_SINGLE_INCLUDE_FIELD.size());
        eventData.getColumns().parallelStream().forEach(eventColumn -> {
            if (null != FieldMapping.GRADE_SINGLE_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.GRADE_SINGLE_INCLUDE_FIELD.get(eventColumn.getColumnName()));
        ...
        eventData.setColumns(Arrays.asList(map.keySet().toArray(new EventColumn[FieldMapping.GRADE_SINGLE_INCLUDE_FIELD.size()])));
