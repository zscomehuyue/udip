...
public class RegistProcessor implements EventProcessor {


    @Override
    public boolean process(EventData eventData, DataMediaPair dataMediaPair) {
        Map<EventColumn, String> map = new ConcurrentHashMap(FieldMapping.RGSE_REGISTER_INCLUDE_FIELD.size());
        eventData.getColumns().parallelStream().forEach(eventColumn -> {
            if (null != FieldMapping.RGSE_REGISTER_INCLUDE_FIELD.get(eventColumn.getColumnName())) {
                eventColumn.setColumnName(FieldMapping.RGSE_REGISTER_INCLUDE_FIELD.get(eventColumn.getColumnName()));
        ...
        eventData.setColumns(Arrays.asList(map.keySet().toArray(new EventColumn[FieldMapping.RGSE_REGISTER_INCLUDE_FIELD.size()])));
