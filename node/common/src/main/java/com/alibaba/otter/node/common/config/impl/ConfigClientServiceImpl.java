...
import com.alibaba.otter.shared.communication.model.config.FindLoadRouteEvent;
...
public class ConfigClientServiceImpl implements InternalConfigClientService, ArbitrateConfig, InitializingBean {
    ...
    private RefreshMemoryMirror<Long, Map<Long, List<WideTable>>> wideTableCache;
    private RefreshMemoryMirror<Long, List<LoadRoute>> loadRouteCache;
    private Map<Long, Long> channelMapping;                                                     // 将pipelineId映射为channelId
    private RefreshMemoryMirror<Long, Node> nodeCache;
    private final static Long LOAD_ROUTE_KEY = 1L;
    ...
    public List<WideTable> findWideTable(Long targetId, Long tableId) {
        return Lists.newArrayList(wideTableCache.get(targetId) == null ? null : wideTableCache.get(targetId).get(tableId));
    }

    @Override
    public List<LoadRoute> findAllLoadRoute() {
        return loadRouteCache.get(LOAD_ROUTE_KEY);
    }
    ...
    public List<Pipeline> getCachedPipelines() {
        List<Pipeline> list = new ArrayList<>();
        channelMapping.keySet().forEach(aLong -> {
            list.add(findPipeline(aLong));
        });
        return list;
    }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        wideTableCache = new RefreshMemoryMirror<Long, Map<Long, List<WideTable>>>(timeout, new ComputeFunction<Long, Map<Long, List<WideTable>>>() {
            @Override
            public Map<Long, List<WideTable>> apply(Long targetId, Map<Long, List<WideTable>> oldValue) {
                return new MapMaker().makeComputingMap(new Function<Long, List<WideTable>>() {
                    public List<WideTable> apply(final Long tableId) {
        ...
        loadRouteCache = new RefreshMemoryMirror<Long, List<LoadRoute>>(timeout, new ComputeFunction<Long, List<LoadRoute>>() {

            public List<LoadRoute> apply(Long key, List<LoadRoute> oldValue) {
                FindLoadRouteEvent event = new FindLoadRouteEvent();
                try {
                    Object obj = nodeCommmunicationClient.callManager(event);
                    if (obj != null && obj instanceof List) {
                        loadRouteCache.put(key, (List<LoadRoute>) obj);
                        return (List<LoadRoute>) obj;
                    }
                } catch (Exception e) {
                    logger.error("call_manager_error", event.toString(), e);
                }
