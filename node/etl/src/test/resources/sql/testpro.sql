...
set @sqlText= CONCAT("insert into udip_retl.retl_buffer( GMT_CREATE, PK_DATA, TABLE_ID, TYPE, FULL_NAME,GMT_MODIFIED)
...
DROP PROCEDURE if EXISTS adduc;

DELIMITER $$
CREATE PROCEDURE adduc(maxCount int)
  begin
    DECLARE i int(11);
    set autocommit= 0;
    SET i = 1;
    ...
    WHILE i <= maxCount DO
      insert ignore into D_USER values (i,'meida2',25,100050,now(),now()) ;
      insert into D_COURSE values (null,'语文',90,i,now(),now()) ;
      insert into D_COURSE values (null,'数学',90,i,now(),now()) ;
      insert into D_COURSE values (null,'化学',90,i,now(),now()) ;
      set i=i+1;
      if i%100=0
        then
        commit;
...
DELIMITER $$
CREATE PROCEDURE addc(maxCount int)
  begin
    DECLARE i int(11);
    set autocommit= 0;
    SET i = 1;
