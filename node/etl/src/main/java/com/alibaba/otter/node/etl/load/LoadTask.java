    ...
    public void run() {
        ...
        while (running) {
            try {
                ...
                Runnable task = new Runnable() {
                    public void run() {
                        ...
                        try {
                            ...
                            if (ConfigHelper.needLoadMedia(getPipeline(), DataMediaType.MYSQL)) {
                                processedContexts = otterLoaderFactory.load(dbBatch);
                            }

                            // 传递给下一个流程
                            etlEventData.setDesc(rowDataPipeDelegate.put(dbBatch, etlEventData.getNextNid()));
                            ...
                            if (!isInterrupt(e)) {
                                LogUtils.log(ERROR, logger, () -> format("[%s] loadWork executor is error! data:%s , error:%s",
                                        pipelineId, etlEventData, e));
                            } else {
                                LogUtils.log(INFO, logger, () -> format("[%s] loadWork executor is interrrupt! data:%s , error:%s",
                                        pipelineId, etlEventData, e));
                            }
                            if (processedContexts != null) {// 说明load成功了，但是通知仲裁器失败了，需要记录下记录到store
                                for (LoadContext context : processedContexts) {
                                    try {
                                        ...
                                        LogUtils.log(ERROR, logger, () -> format("[%s] interceptor process error failed!, error:%s",
                                                pipelineId, ie));
                                    }
                                }
                            }
                ...
                if (isInterrupt(e)) {
                    LogUtils.log(INFO, logger, () -> "[%s] loadTask is interrupted! , error:%s", pipelineId, e);
                    // arbitrateEventService.loadEvent().release(pipelineId); //
                    // 释放锁
                    return;
                } else {
                    LogUtils.log(ERROR, logger, () -> "[%s] loadTask is error: %s", pipelineId, e);
                    // arbitrateEventService.loadEvent().release(pipelineId); //
                    // 释放锁
