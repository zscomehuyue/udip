...
import com.alibaba.otter.node.etl.load.IndexLoadTask;
...
public class OtterController implements NodeTaskListener, OtterControllerMBean {
    ...
    private void startTask(Pipeline pipeline, Map<StageType, GlobalTask> tasks, StageType taskType) {
        ...
        } else if (taskType.isIndexLoad()) {
            task = new IndexLoadTask(pipeline.getId());
        } /*else if (taskType.isMqLoad()) {FIXME MQ
            task = new MqLoadTask(pipeline.getId());
        }*/
