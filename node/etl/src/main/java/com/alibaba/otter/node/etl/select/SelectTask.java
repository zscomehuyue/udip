...
public class SelectTask extends GlobalTask {
        ...
        while (running) {
            try {
                ...
                if (rversion.get() != startVersion) {// 说明存在过变化，中间出现过rollback，需要丢弃该数据
                    LogUtils.log(WARN, logger, () -> "rollback happend , should skip this data and get new message.");
                ...
                Runnable task = new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            LogUtils.log(INFO, logger, () -> "=processSelect=>rowBatch=" + ToStringBuilder.reflectionToString(rowBatch));
                            if (profiling) {
    ...
    private boolean processTermin(boolean lastStatus, Long batchId, Long processId) throws InterruptedException {
        ...
        while (retry++ < 60) {
            ...
            if (terminBatchId == null && processId != -1L && !processId.equals(terminProcessId)) {
                ...
                exception = new SelectException("unmatched processId, SelectTask batchId = " + batchId
                        ...
                        + terminData.toString());
                log(WARN,logger,()->"=ack=>unmatched: batchId:%s ,processId:%s ,terminData:%s ",batchId,processId,terminData.toString());
        ...
        if (exception != null) {
             log(WARN,logger,()->"=ack=>error: batchId:%s ,processId:%s ,terminData:%s  ",batchId,processId,terminData.toString());
            throw exception;
        }
