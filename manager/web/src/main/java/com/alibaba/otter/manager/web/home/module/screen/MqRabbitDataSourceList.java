...
import com.alibaba.otter.manager.web.common.model.SeniorMqDataMediaSource;
...
public class MqRabbitDataSourceList {
    ...
    public void execute(@Param("pageIndex") int pageIndex, @Param("searchKey") String searchKey, Context context)
        ...
        // 显示ES数据
        List<DataMediaType> typeLs = new ArrayList<>();
        typeLs.add(DataMediaType.RABBIT);
        condition.put("DataMediaType", typeLs);


        List<DataMediaSource> dataMediaSources = dataMediaSourceService.listByCondition(condition);
        List<SeniorMqDataMediaSource> seniorDataMediaSources = new ArrayList<SeniorMqDataMediaSource>();
        for (DataMediaSource dataMediaSource : dataMediaSources) {

            SeniorMqDataMediaSource seniorDataMediaSource = new SeniorMqDataMediaSource();
            ...
            if (dataMediaSource instanceof MqMediaSource) {
                MqMediaSource ms = (MqMediaSource)dataMediaSource;
                seniorDataMediaSource.setHost(ms.getHost());
                seniorDataMediaSource.setPort(ms.getPort());
                seniorDataMediaSource.setVhost(ms.getVhost());
                seniorDataMediaSource.setUserName(ms.getUserName());
                seniorDataMediaSource.setPassword(ms.getPassword());
            }
