...
import com.alibaba.otter.shared.common.model.config.data.es.IndexDataMedia;
...
public class DataMediaServiceImpl implements DataMediaService {
    ...
    public void create(DataMedia dataMedia) {
        ...
        if (dataMedia.getName().contains(",")) {
            String names = dataMedia.getName();
            Arrays.stream(names.split(",")).forEach(name -> {
                dataMedia.setName(name);
                create(dataMedia);
            });
        } else {
            createTable(dataMedia);
        }
    }

    private void createTable(DataMedia dataMedia) {
        try {
    ...
    private DataMedia doToModel(DataMediaDO dataMediaDo) {
        ...
        try {
            ...
            if (dataMediaSource.getType().isMysql() || dataMediaSource.getType().isOracle()) {
                ...
                dataMedia.setSource(dataMediaSource);
            } else if (dataMediaSource.getType().isIndex()) {
                dataMedia = JsonUtils.unmarshalFromString(dataMediaDo.getProperties(), IndexDataMedia.class);
                dataMedia.setSource(dataMediaSource);
            }
            ...
            logger.error("=doToModel=>error change the dataMedia Do to Model has an exception , DataMediaSourceId={}", dataMediaDo.getDataMediaSourceId());
            throw new ManagerException(e);
        }
